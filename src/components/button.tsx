...
export const Button = (props: ButtonProps) => {
  const [p, customProps] = splitProps(props, ["text", "onclick", "color", "class"]);
  const uniqueId = createUniqueId();
  return (
    <button
      type="button"
      className={
        "inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-full shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 " +
        p.class
      }
      classList={{
